...
class APICacheFile extends APICacheEngine
{
    ...
    {
        if (!is_dir(API_TRUST_PATH . '/' . parse_url(API_URL, PHP_URL_HOST)))
            mkdir(API_TRUST_PATH . '/' . parse_url(API_URL, PHP_URL_HOST), 0777, true);
        
        parent::init($settings);
        $defaults       = array(
            'path'      => API_TRUST_PATH . '/' . parse_url(API_URL, PHP_URL_HOST),
            'extension' => '.php',
            'prefix'    => 'api_',
            'lock'      => false,
            'serialize' => false,
